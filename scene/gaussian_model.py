...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        if self._embedding_dim > 0:
            print(f"Pre-Embedding of {self._embedding_dim} dimensions")
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # if self._embedding_dim > 0:
        #     self._embedding = nn.Parameter(torch.randn((self.get_xyz.shape[0], self._embedding_dim), device='cuda').requires_grad_(True)* 0.1)
        # else:
        #     self._embedding = torch.zeros((self.get_xyz.shape[0], self._embedding_dim), device='cuda')
        # self._embedding = nn.Parameter(torch.randn((self.get_xyz.shape[0], self._embedding_dim), device='cuda').requires_grad_(True)* 0.1)

        embedding = torch.zeros((fused_color.shape[0], self._embedding_dim)).float().cuda()  # [jm]
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._embedding], 'lr': training_args.feature_lr, "name": "embeding"}
        ]
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._embedding.shape[1]):
            l.append('embedding_{}'.format(i)) # 加入embedding
        return l
    ...
    def save_deformed_ply(self, xyz, scale, rotation, path):
        ...
        embeding = embeding.detach().cpu().numpy()
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, embeding), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def load_ply(self, path):
        ...
        # embeding = np.randn((xyz.shape[0], self._embedding_dim)) # mine
    ...
    def prune_points(self, mask):
        ...
        self._embedding = optimizable_tensors["embeding"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_identity, new_opacities, new_scaling, new_rotation, new_embedding):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_identity, new_opacity, new_scaling, new_rotation, new_embedding)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_identity, new_opacities, new_scaling, new_rotation, new_embedding)
