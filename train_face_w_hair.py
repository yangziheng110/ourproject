...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        face_only_mask = torch.as_tensor(viewpoint_cam.talking_dict["face_only_mask"]).cuda()
        # head_mask =  face_mask + hair_mask + neck_mask
        head_mask = face_only_mask 
        ...
        gt_image  = viewpoint_cam.original_image.cuda() / 255.0 # 原始图片
        gt_image_white = gt_image * head_mask + background[:, None, None] * ~head_mask # GT图片去掉head_mask部分，加上背景无mask部分进行计算
        ...
        if iteration < bg_iter:
            ...
            gt_image_white[:, mouth_mask] = background[:, None] # mouth的mask部分变为背景

            Ll1 = l1_loss(image_white, gt_image_white) # l1 loss
            ...
            if  iteration > warm_step + 2000:
            
                loss += 1e-4 * (1 - viewpoint_cam.talking_dict["normal"].cuda() * render_pkg["normal"]).sum(0)[head_mask^mouth_mask].mean()
                #print("normal")
                if iteration % opt.opacity_reset_interval > 100:
                    
                    depth = render_pkg["depth"][0]
                    depth_mono = viewpoint_cam.talking_dict['depth'].cuda()
                    loss += 1e-4 * (normalize(depth)[face_mask^mouth_mask] - normalize(depth_mono)[face_mask^mouth_mask]).abs().mean()
