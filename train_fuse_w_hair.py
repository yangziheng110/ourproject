...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians.restore(model_params, opt) # 设置高斯参数
    motion_net.load_state_dict(motion_params) # 导入模型参数

    (model_params, motion_params, _, _) = torch.load(os.path.join(scene.model_path, "chkpnt_mouth_latest.pth"))
    gaussians_mouth.restore(model_params, opt) # 高斯参数
    motion_net_mouth.load_state_dict(motion_params) # 导入模型参数
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration > bg_iter:
            ...
            for param in motion_net_mouth.parameters():
                param.requires_grad = False
            gaussians._embedding.requires_grad = False
        ...
        if iteration < bg_iter:            
            ...
            Ll1 = l1_loss(image, gt_image) # 默认直接 与 gt_image 进行 计算
